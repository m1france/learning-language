...
function saveSyncState(payload) {
    try {
        ...
        try {
            ...
            writeFileSync(SYNC_FILE, JSON.stringify(data, null, 2), 'utf-8');
        }
        return nextRevision;
    }
    catch (err) {
...
function saveFileLessons(lessons) {
    try {
...
app.post('/api/share/lessons', async (c) => {
    try {
        ...
        if (dbPool) {
            try {
                await dbPool.query('INSERT INTO shared_lessons (id, username, data) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data), username = VALUES(username)', [lesson.id, lesson.username, JSON.stringify(lesson)]);
            }
            catch (err) {
...
app.get('/api/share/lessons', async (c) => {
    ...
    if (dbPool) {
        try {
            ...
            const list = rows.map((r) => (typeof r.data === 'string' ? JSON.parse(r.data) : r.data));
...
app.post('/api/share/lessons/:id/reactions', async (c) => {
    ...
    const lesson = fileLessons[id];
    if (!lesson)
        return c.json({ error: 'Leçon introuvable' }, 404);
...
app.post('/api/share/lessons/:id/stickers', async (c) => {
    ...
    if (!id || !emoji)
        return c.json({ error: 'Sticker manquant' }, 400);
    const fileLessons = loadFileLessons();
    const lesson = fileLessons[id];
    if (!lesson)
        return c.json({ error: 'Leçon introuvable' }, 404);
